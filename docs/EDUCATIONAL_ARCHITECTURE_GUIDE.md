...
- **Production artifact naming.** v1 (`rhythm-skeleton-mt3-v1`) and v2 (`rhythm-skeleton-mt3-v2`) are both private HuggingFace repos under `lukasnorland`. Pin a specific version in any writeup; this repo’s default is v2 via `_GAME_CODE_REGISTRY["MT3"]` in [`snapbeat_inference.py`](../snapbeat_inference.py), and v1 can be selected explicitly with `lora_path=...` for A/B comparisons.
